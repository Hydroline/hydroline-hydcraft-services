...
async function main() {
    ...
    while (true) {
      ...
      const tasks = batch.map((row) =>
        limit(async () => {
          ...
          try {
            const stream = await s3.getObjectStream(row.storageKey);
          ...
          const verify = await local.headObject(row.storageKey);
          if (!verify.exists) {
            missingLocal++;
          }
        }),
      ...
      console.log(
        `[s3->local] scanned=${scanned} downloaded=${downloaded} exists=${skippedExists} missingS3=${missingS3} failed=${failed}`,
      );
    }
  } finally {
  ...
  // eslint-disable-next-line no-console
  console.log(
    `[s3->local] DONE scanned=${scanned} downloaded=${downloaded} exists=${skippedExists} missingS3=${missingS3} missingLocal=${missingLocal} failed=${failed}`,
  );
}
