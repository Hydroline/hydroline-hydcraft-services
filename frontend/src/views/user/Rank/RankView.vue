...
const syncStatusLabel = computed(() => {
  ...
  if (selectedServer.value?.lastSyncedAt) {
    return `最近同步：${formatTimestamp(selectedServer.value.lastSyncedAt)}`
  }
  return '尚未同步'
})
    ...
    <div>
      ...
      >
        <div class="overflow-x-auto" ref="tableScrollContainer">
          ...
          >
            <thead
              class="bg-slate-50 text-xs uppercase tracking-wide whitespace-nowrap text-slate-500 dark:bg-slate-800/60 dark:text-slate-400"
            >
              <tr>
                <th
                  class="sticky left-0 z-20 px-4 py-3 text-left text-slate-600 bg-slate-50 dark:text-slate-300 dark:bg-slate-800/60"
                  style="width: 3.5rem; min-width: 3.5rem"
                >
                  #
                </th>
                <th
                  class="sticky left-14 z-20 px-4 py-3 text-left text-slate-600 bg-slate-50 dark:text-slate-300 dark:bg-slate-800/60"
            ...
            <tbody>
              ...
              <template v-else>
                <tr
                  ...
                  :key="entry.playerUuid ?? `${entry.rank}-${entry.playerName}`"
                  class="border-t border-slate-100 odd:bg-white even:bg-slate-50 dark:border-slate-800 dark:odd:bg-slate-900 dark:even:bg-slate-850"
                >
                  <td
